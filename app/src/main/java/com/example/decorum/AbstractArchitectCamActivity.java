 ...
 *
 */
public abstract class AbstractArchitectCamActivity extends Activity implements com.example.decorum.ArchitectViewHolderInterface {
			...
			this.locationListener = new LocationListener() {
				...
				public void onLocationChanged( final Location location ) {
					...
					// sore last location as member, in case it is needed somewhere (in e.g. your adjusted project)
						com.example.decorum.AbstractArchitectCamActivity.this.lastKnownLocaton = location;
						if ( com.example.decorum.AbstractArchitectCamActivity.this.architectView != null ) {
							// check if location has altitude at certain accuracy level & call right architect method (the one with altitude information)
							if ( location.hasAltitude() && location.hasAccuracy() && location.getAccuracy()<7) {
								com.example.decorum.AbstractArchitectCamActivity.this.architectView.setLocation( location.getLatitude(), location.getLongitude(), location.getAltitude(), location.getAccuracy() );
							} else {
								com.example.decorum.AbstractArchitectCamActivity.this.architectView.setLocation( location.getLatitude(), location.getLongitude(), location.hasAccuracy() ? location.getAccuracy() : 1000 );
							}
						}
					}
				}
	...
	protected void onPostCreate( final Bundle savedInstanceState ) {
		...
		if ( this.architectView != null ) {
			...
			try {
				...
				if (this.getInitialCullingDistanceMeters() != com.example.decorum.ArchitectViewHolderInterface.CULLING_DISTANCE_DEFAULT_METERS) {
	...
	protected void onStop() {
		Log.d("AbstartedArchitect", "onStop");
		super.onStop();
	}

	@Override
	protected void onDestroy() {
		Log.d("AbstartedArchitect", "onDestry");
		super.onDestroy();
				...
				public void run() {
					...
					while (lastKnownLocaton==null && !isFinishing()) {
						...
						runOnUiThread(new Runnable() {
							...
							@Override
							public void run() {
								Toast.makeText(com.example.decorum.AbstractArchitectCamActivity.this, R.string.location_fetching, Toast.LENGTH_SHORT).show();
							}
						});
